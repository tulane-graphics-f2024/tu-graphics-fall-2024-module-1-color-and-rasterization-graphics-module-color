...
enum{_TRIANGLE, _SQUARE, _OUTLINED_SQUARE, _NUM_OBJ};
int current_object;
...
void init(GLuint &glShaderProgram, GLuint &triVao, GLuint &sqVao){
  ...
  square[0] = vec2(-0.5, -0.5);
  square[1] = vec2(-0.5,  0.5);
  square[2] = vec2( 0.5, -0.5);
  ...
  square_colors[3] = vec3( 1.0, 0.0, 0.0);
  size_t square_color_bytes = 4*sizeof(vec3);
  ...
  //Second part of array hold colors (offset by sizeof(triangle))
  glBufferSubData( GL_ARRAY_BUFFER, square_bytes, square_color_bytes, square_colors );
  
  glEnableVertexAttribArray(vpos_location);
  glEnableVertexAttribArray(vcolor_location);
  
  glVertexAttribPointer( vpos_location, 2, GL_FLOAT, GL_FALSE, 0, BUFFER_OFFSET(0) );
  glVertexAttribPointer( vcolor_location, 3, GL_FLOAT, GL_FALSE, 0, BUFFER_OFFSET(square_bytes) );
